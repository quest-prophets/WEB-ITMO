<template>
    <div class="grid--graphPanel flexColumn">
        <div class="flexColumn graphPanel">
            <div>R: &emsp;<input v-model="r" type="number" step="0.1" min="1" max="5" placeholder="1..5"
                                 class="graphInput"></div>
            <div>X: &emsp;<input v-model="x" type="number" step="0.1" min="-5" max="3" placeholder="-5..3"
                                 class="graphInput"></div>
            <div>Y: &emsp;<input v-model="y" type="number" step="0.1" min="-3" max="3" placeholder="-3..3"
                                 class="graphInput"></div>
            <button @click="logToConsole" class="bwButton bwButton-blackBackground">Focus</button>
            <button class="bwButton bwButton-blackBackground">Erase</button>
        </div>
    </div>
</template>

<script>
    export default {
        name: "GraphPanel",
        data: function () {
            return {r: null, x: null, y: null}
        },
        methods: {
            logToConsole: function () {
                console.log(this.r + " " + this.x + ' ' + this.y);
                this.$emit("add-data", {r: this.r, x: this.x, y: this.y})
            }
        }
    }
</script>

<style>
    .bwButton {
        width: 100px;
        padding: 7px 12px;
        color: white;
        background-color: black;
        font-weight: bold;
        margin-top: 10px;
        cursor: pointer;
        outline: none;
    }

    .bwButton-blackBackground {
        border: 2px solid white;
    }

    .bwButton-blackBackground:hover {
        background: white;
        color: black;
    }

    .flexColumn {
        display: flex;
        flex-direction: column;
        justify-content: center;
        align-items: center;
    }

    .grid--graphPanel {
        grid-area: graphPanel;
    }

    .graphPanel {
        color: white;
        border: 2px solid white;
        padding: 20px 30px;
        border-radius: 15px;
        background: rgba(0, 0, 0, 0.6);
        box-shadow: 0 0 10px 10px rgba(0, 0, 0, 0.6);
    }

    .graphInput {
        color: white;
        background-color: black;
        padding: 8px 12px;
        font-weight: bold;
        margin-top: 10px;
        border-radius: 6px;
        border: 2px solid white;
        width: 7vw;
    }
</style>